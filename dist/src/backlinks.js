...
export class BacklinksService {
    ...
    async findBacklinks(noteName) {
        ...
        const linkPattern = new RegExp(`\\[\\[${this.escapeRegex(normalizedName)}(?:\\|[^\\]]+)?\\]\\]`, 'gi');
        try {
            // Get all markdown files
            const allFiles = await this.getAllMarkdownFiles(this.vaultPath);
            // Search each file
            for (const filePath of allFiles) {
                const matches = await this.searchFileForBacklinks(filePath, linkPattern);
            ...
            throw new Error(`Failed to find backlinks: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     * Search a single file for backlinks to the target note
     */
    async searchFileForBacklinks(filePath, linkPattern) {
        try {
            ...
            const contextLines = matches.slice(0, 3).map(m => `Line ${m.line}: ${m.text.substring(0, 150)}${m.text.length > 150 ? '...' : ''}`);
            if (matches.length > 3) {
